var validationMessages = {
    ...
    phoneru:{required: "Вы пропустили", phoneua:"Не валиднй номер"},
    ...
    passruser:{required: "Вы пропустили", number:"Только цифри",minlength: "минимум 4", maxlength: "превышает 4" },
    inn:{required: "Вы пропустили", number:"Только цифри",minlength: "минимум 10", maxlength: "превышает 10" },
    innru:{required: "Вы пропустили", number:"Только цифри",minlength: "минимум 12", maxlength: "превышает 12" },
    ...
    itemname:{required: "Вы пропустили", maxlength: "превышает 200" },
    ...
    if ($form.length && $form.attr('novalidate') === undefined) {
        ...
        valic = $form.validate({
            rules:{
                ...
                phoneru:{required: true, phoneru:true},
                ...
                passruser:{required: true, number:true, minlength: 4, maxlength: 4 },
                inn:{required: true, number:true, minlength: 10, maxlength: 10 },
                innru:{required: true, number:true, minlength: 12, maxlength: 12 },
                ...
                itemname:{required:true, maxlength:200},
                tracknumber:{ maxlength:50, tracknumber:true}
            },
...
$.validator.addMethod("tracknumber", function(value, element) {
    return this.optional(element) || /^[A-z\d_']+$/g.test(value);
}, "Please specify the correct track number");
...
$.validator.addMethod("phoneru", function(value, element){
    return this.optional(element) || /\+7\([\d]{3}\) [\d]{3}\-[\d]{2}\-[\d]{2}/g.test(value);
}, "Please specify the correct UA phone");
...
$.validator.addMethod("birthday", function(value, element){
    ...
    if(elems.length == 0 ){
        ...
        var today = new Date();
        if ((today.getFullYear() - bd.getFullYear()) < 18){ return false}
        else if ((today.getFullYear() - bd.getFullYear()) == 18){
            if ((today.getMonth()) < bd.getMonth()){ return false}
            else if ((today.getMonth()) == bd.getMonth()){
                if(today.getDate() < bd.getDate() - 1){return false;}
            }
        }
        return true;
    }
...
$.validator.addMethod("checkbd", function(value, element){
    ...
    if(elems.length == 0 && elems2.length == 0){
        ...
        if ((thisdate.getFullYear() - bd.getFullYear()) < 16){ return false}
        else if ((thisdate.getFullYear() - bd.getFullYear()) == 16){
            if ((thisdate.getMonth()) < bd.getMonth()){ return false}
            else if ((thisdate.getMonth()) == bd.getMonth()){
                if(thisdate.getDate()  < bd.getDate() ){return false;}
            }
        }
        return true;
    }
